...
describe("Mail Filters", () => {
	...
	it("handles Send Date filters", () => {
		const runSendDateFilter = () => {
			wrapper.find("#send-date").first().simulate("click");
	...
	it("clicking on the 'Send Mail' button, moves the user to the Send Mail Form page", () => {
		wrapper.find("Button.send-mail").first().simulate("click");

		expect(Router.push).toHaveBeenCalledWith("/employee/mail/create");
	});
});
